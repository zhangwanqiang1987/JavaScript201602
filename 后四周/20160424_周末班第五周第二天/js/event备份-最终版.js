...
function processThis(callBack, context) {
    var outerArg = Array.prototype.slice.call(arguments, 2);//outerArg是大函数里的e  2是参数第2项
    return function () {
        var innerArg = Array.prototype.slice.call(arguments, 0);//innerArgue是小函数里的e  0是参数第一项
...
function fire(e) {
    ...
    if (ary) {
        for (var i = 0; i < ary.length; i++) {
            ...
            if (typeof curFn === "function") {//只有在数组塌陷里面是function 的哪几项去改变this 下面的else是把数组塌陷的那几项为null的删除
                curFn.call(this, e);
            } else {
